...
const BOARD_SIZE = 15;
function getTile(board, boardPosition) {
 ...
 */
function getDisplayLetter(tile) {
...
// ----------------------------------------
export function findHorizontalWord(board, startPos) {
    ...
    const wordTiles = [];
    const startOfWordPos = { row, col: startIndex };
...
export function findVerticalWord(board, startPos) {
    if (getTile(board, startPos) === null) {
        return null;
    }
    ...
    // On doit aussi garder une trace des tuiles et de la position de départ
    const wordTiles = [];
    const startOfWordPos = { row: startIndex, col };
...
export function findAllWordsFormedByMove(board, placedTiles) {
