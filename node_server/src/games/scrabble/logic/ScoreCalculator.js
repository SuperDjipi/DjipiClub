...
function getBonus(board, pos) {
    return board[pos.row]?.[pos.col]?.bonus ?? 'NONE';
}
function calculateWordScore(word, board, newlyPlacedPositions) {
    let wordScore = 0;
...
export function calculateTotalScore(foundWords, board, newlyPlacedPositions) {
    let totalScore = 0;
