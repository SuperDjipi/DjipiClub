...
import type { GameState, Tile, PlacedTile, ScrabblePlayer, Board } from '../models/GameModels.js';
...
export function processExchangeTiles(game: GameState, player: ScrabblePlayer, tilesToExchange: Tile[]): GameState {
    ...
    const tileIdsToExchange = new Set(tilesToExchange.map((t: Tile) => t.id));
    // 2. LOGIQUE D'ÉCHANGE
    // Séparer les tuiles à garder et celles à échanger
    const tilesToKeep = player.rack.filter((t: Tile) => !tileIdsToExchange.has(t.id));
    const tilesToReturn = player.rack.filter((t: Tile) => tileIdsToExchange.has(t.id));
