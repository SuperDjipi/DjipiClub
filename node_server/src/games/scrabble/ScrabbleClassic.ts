...
import { GameType, GameStatus, type Player, type MoveResult } from '../../models/games/IGame.js';
import type { Tile, Board, PlacedTile, BoardPosition, ScrabblePlayer } from './models/GameModels.js';
...
export class ScrabbleClassic extends BaseGame {
    ...
    // Override players avec le type ScrabblePlayer
    declare players: ScrabblePlayer[];

    // État spécifique Scrabble
    board: Board;
    tileBag: Tile[];
    placedPositions: BoardPosition[] = [];
    forceEndGame: number = 0;
    ...
    handleMove(playerId: string, move: any): MoveResult {
        ...
        const placedTiles: PlacedTile[] = move.placedTiles || [];
    ...
    private _applyMove(placedTiles: PlacedTile[], playerId: string): void {
        ...
        if (player && player.rack) {
            ...
            player.rack = player.rack.filter((t: Tile) => !usedTileIds.includes(t.id));
    ...
    override addPlayer(player: Player): boolean {
        ...
        const scrabblePlayer: ScrabblePlayer = {
            ...player,
            rack: []
        };
